...
import { Barbershop, BarbershopService, Booking, Prisma, User } from "@prisma/client"
...
import { deleteService } from "@/app/_data/barbershop-service/delete-service"
import { XIcon } from "lucide-react"
import { getServerSession } from "next-auth"
import { authOptions } from "@/app/_lib/auth"
import Link from "next/link"
import { updateService } from "@/app/_data/barbershop-service/update-service"
import { Input } from "@/app/_components/ui/input"; // Um input estilizado
...
const ServiceItem = ({ service, barbershop }: ServiceItemProps) => {
  ...
  const handleDeleteService = async () => {
    try {
      await deleteService(service.id, barbershop.id)
  ...
  const handleUpdateService = async () => {
    try {
      ...
      router.refresh(); // Atualiza os dados na página
  ...
  return (
    <>
            ...
            <div className="flex gap-3">
              {isOwner && (
                <Button
                  variant="secondary"
                  size="sm"
                  onClick={() => setEditDialogIsOpen(true)} // Abre o modal de edição
                >
                  Editar
                </Button>
      ...
      {/* {isOwner && (
        <Button variant="secondary" size="sm" onClick={() => setEditDialogIsOpen(true)}>
          Editar
        </Button>
      )} */}
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <Input
              name="Preço"
              type="number"
              value={Number(serviceData.price)}
              onChange={(e) => handleInputChange("price", Number(e.target.value))}
            />
          </div>
          <DialogFooter>
            <Button variant="secondary" onClick={() => setEditDialogIsOpen(false)}>
              Cancelar
            </Button>
      ...
      <Sheet open={editDialogIsOpen} onOpenChange={setEditDialogIsOpen}>
